...
const Instructions = () => {
  return (
    <div className="w-full h-full px-0">
      ...
      <ul className="space-y-6 lg:space-y-0 lg:space-x-6 px-6 md:px-10 lg:px-[7rem] mt-20 lg:mt-16 2xl:mt-24 pb-6 lg:grid lg:grid-cols-3">
        {instructionList.map((item, index) => (
          <li
            className="min-h-[11.6rem] lg:min-h-[34.5rem]  rounded-[2rem] bg-white p-8 lg:p-0 space-y-2 md:space-y-0 md:flex md:items-center md:space-x-8 lg:space-x-0 "
            key={item.title}
          >
            ...
            <div className="space-x-4 text-7xl capitalize hidden  text-[#2463FF] lg:block lg:text-[5.5rem] ">
              ...
              <p className="text-[#887DC0] w-12/12 md:text-xl lg:text-[1.625rem] lg:leading-9 lg:w-9/12 ">{item.text}</p>
            </div>
